...
export const streamChat = async (
  ...
  if (!PROXY_URL) {
    handlers.onError('Falta configurar EXPO_PUBLIC_CHAT_PROXY_URL (URL del proxy de chat).');
  ...
  es.addEventListener('error', (event) => {
    const msg =
      'message' in event && event.message ? event.message : 'No se pudo conectar con el asistente.';
