...
export const HabitsScreen = () => {
  ...
  const freezeStreak = useProductivityStore((state) => state.freezeStreak);
  const removeHabit = useProductivityStore((state) => state.removeHabit);
    ...
    <>
      ...
      <View style={styles.filters} accessibilityRole="tablist">
        {(['today', 'all'] as const).map((value) => {
          ...
          const count = value === 'today' ? todayHabits.length : habits.length;
          return (
            <TouchableOpacity
              key={value}
            ...
            >
              <Text style={[styles.filterText, selected && styles.filterTextActive]}>
                {t(value === 'today' ? 'habits.today' : 'habits.mine')}
              </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyCard}>
            ...
            {filter === 'today' && habits.length ? (
              <TouchableOpacity style={styles.emptyAction} onPress={() => setFilter('all')}>
                <Text style={styles.emptyActionText}>{t('habits.viewMine')}</Text>
              </TouchableOpacity>
            ) : null}
          </View>
        }
