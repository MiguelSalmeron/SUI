# Documentación de SUI

La documentación sigue el modelo Diátaxis: aprendizaje, tareas, referencia y explicación.

## Tutoriales

- [Primeros pasos](tutorials/getting-started.md)

## Guías prácticas

- [Configurar Firebase](how-to/firebase-config.md)
- [Completar configuración cloud](how-to/complete-cloud-configuration.md)
- [Desplegar el proxy de chat](how-to/deploy-chat-proxy.md)

## Referencia

- [Guía del desarrollador](reference/developer-guide.md)
- [API, persistencia y tema](reference/api-and-theme.md)
- [Ejemplo de configuración de crisis](reference/examples/crisis-config.json)

## Explicaciones

- [Arquitectura](explanation/architecture.md)
- [Chatbot](explanation/chatbot.md)
- [Privacidad y crisis](explanation/privacy-and-crisis.md)
- [Visión del producto](explanation/product-overview.md)

## Decisiones arquitectónicas

- [ADR-0001: arquitectura orientada por funcionalidad](decisions/0001-feature-oriented-architecture.md)
- [ADR-0002: conservar React Navigation](decisions/0002-keep-react-navigation.md)
- [ADR-0003: umbral para adoptar monorepo](decisions/0003-monorepo-threshold.md)

## Planificación

- [Roadmap](roadmap.md)
- [Archivo histórico](archive/README.md)
