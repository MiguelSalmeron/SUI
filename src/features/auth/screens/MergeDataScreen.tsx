...
export const MergeDataScreen = ({ navigation }: Props) => {
  ...
  const confirmCloud = () => {
    Alert.alert(t('merge.cloudConfirmTitle'), t('merge.cloudConfirmBody'), [
      ...
      { text: t('merge.cloudConfirmAction'), style: 'destructive', onPress: () => void resolve('cloud') },
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.primary} onPress={() => void resolve('combine')} disabled={busy !== null}>
          {busy === 'combine' ? <ActivityIndicator color={theme.colors.onPrimary} /> : <Ionicons name="git-merge-outline" size={21} color={theme.colors.onPrimary} />}
        ...
        <TouchableOpacity style={styles.secondary} onPress={confirmCloud} disabled={busy !== null}>
          {busy === 'cloud' ? <ActivityIndicator color={theme.colors.primary} /> : <Ionicons name="cloud-download-outline" size={21} color={theme.colors.primary} />}
          <View style={styles.buttonCopy}>
...
const createStyles = ({ colors, radius, type }: AppTheme) => StyleSheet.create({
  ...
  title: { ...type.headlineSm, color: colors.onSurface, textAlign: 'center', marginTop: SPACING.lg },
  body: { ...type.bodyMd, color: colors.onSurfaceVariant, textAlign: 'center', marginTop: SPACING.xs, marginBottom: SPACING.xl },
  primary: { width: '100%', minHeight: 72, borderRadius: radius.lg, backgroundColor: colors.primary, flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, gap: SPACING.md },
  secondary: { width: '100%', minHeight: 72, borderRadius: radius.lg, backgroundColor: colors.surfaceContainer, borderWidth: 1, borderColor: colors.outlineVariant, flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, gap: SPACING.md, marginTop: SPACING.md },
  buttonCopy: { flex: 1 },
